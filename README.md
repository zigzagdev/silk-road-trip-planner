# Silk Road Trip Planner 🐪

A 14-day journey through Central Asia — tracing the ancient Silk Road across Uzbekistan, Kazakhstan, and Kyrgyzstan.

## Trip Overview

- **Period**: September 16–29
- **Duration**: 14 days (including travel days)
- **Travelers**: 2
- **Theme**: UNESCO World Heritage Sites, nomadic culture, natural landscapes
- **Route**: Uzbekistan → Kazakhstan → Kyrgyzstan → Uzbekistan

## Route Summary

| Country | City | Highlights |
|---|---|---|
| 🇺🇿 Uzbekistan | Tashkent / Samarkand / Bukhara / Khiva | UNESCO World Heritage Sites (the heaviest part of the trip) |
| 🇰🇿 Kazakhstan | Almaty | Former capital, mountains & city |
| 🇰🇬 Kyrgyzstan | TBD | Nature, mountains, nomadic life |

## Itinerary

| Day | Date | Theme | Country | File |
|---|---|---|---|---|
| 1 | 9/16 | Arrival & Tashkent | 🇺🇿 | `day1-tashkent.md` |
| 2 | 9/17 | Samarkand — Registan & Shah-i-Zinda | 🇺🇿 | `day2-samarkand.md` |
| 3 | 9/18 | Samarkand → Bukhara | 🇺🇿 | `day3-bukhara.md` |
| 4 | 9/19 | Bukhara — Ark Fortress & Kalon Minaret | 🇺🇿 | `day4-bukhara.md` |
| 5 | 9/20 | Bukhara → Khiva | 🇺🇿 | `day5-khiva.md` |
| 6 | 9/21 | Khiva → Almaty | 🇰🇿 | `day6-almaty.md` |
| 7 | 9/22 | Almaty → Kyrgyzstan | 🇰🇬 | `day7-kyrgyzstan.md` |
| 8 | 9/23 | Nature & Mountains | 🇰🇬 | `day8-kyrgyzstan.md` |
| 9 | 9/24 | TBD | 🇰🇬 | `day9-tbd.md` |
| 10 | 9/25 | TBD | TBD | `day10-tbd.md` |
| 11 | 9/26 | TBD | TBD | `day11-tbd.md` |
| 12 | 9/27 | TBD | TBD | `day12-tbd.md` |
| 13 | 9/28 | Departure flight | — | `day13-departure.md` |
| 14 | 9/29 | Return to Japan (arrival 20:00) | 🇯🇵 | `day14-return.md` |

## Other Files

| File | Contents |
|---|---|
| `accommodation.md` | Hotel & accommodation summary |
| `budget.md` | Budget estimate & currency info |
| `checklist.md` | Packing list & visa requirements |
| `food.md` | Central Asian food guide (plov, lagman, samsa...) |
| `risks.md` | Travel risks & notes |

## Notes

- Uzbekistan is the highlight — allocate the most time here
- Kazakhstan is a transit stop (Almaty only)
- Kyrgyzstan is for nature and decompression
- Visa requirements vary by country — check in advance